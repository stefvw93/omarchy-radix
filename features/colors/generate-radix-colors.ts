...
export const generateRadixColors = ({
	...
	accentScaleColors[10].coords[1] = Math.min(
		Math.max(
			accentScaleColors[8]!.coords[1]!,
			accentScaleColors[7]!.coords[1]!,
		),
...
function getButtonHoverColor(source: Color, scales: ArrayOf12<Color>[]) {
	const [L, C, H] = source.coords as [number, number, number];
...
function getTextColor(background: Color) {
	...
	if (Math.abs(white.contrastAPCA(background)) < 40) {
		const [, C, H] = background.coords as [number, number, number];
...
function getAlphaColorSrgb(
	...
	const [r, g, b, a] = getAlphaColor(
		new Color(targetColor).to("srgb").coords as [number, number, number],
		new Color(backgroundColor).to("srgb").coords as [number, number, number],
		255,
		255,
		targetAlpha,
	);
...
function getAlphaColorP3(
	...
	const [r, g, b, a] = getAlphaColor(
		new Color(targetColor).to("p3").coords as [number, number, number],
		new Color(backgroundColor).to("p3").coords as [number, number, number],
